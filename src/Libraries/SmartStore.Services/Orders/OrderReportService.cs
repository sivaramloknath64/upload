    ...
    public partial class OrderReportService : IOrderReportService
    {
            ...
            string billingEmail, bool ignoreCancelledOrders = false)
        {
			...
			var item = (from oq in query
						...
						select new { OrderTaxSum = result.Sum(o => o.OrderTax), OrderTotalSum = result.Sum(o => o.OrderTotal) }
					   ).Select(r => new OrderAverageReportLine(){ SumTax = r.OrderTaxSum, SumOrders = r.OrderTotalSum}).FirstOrDefault();
